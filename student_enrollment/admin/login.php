<?php
session_start();
require_once 'db_con.php'; // Include the database connection file
...
if (isset($_POST['login'])) {
	...
	if (count($input_arr) == 0) {
		...
		if ($result) {
			if (mysqli_num_rows($result) == 1) {
				...
				if ($row['password'] == sha1(md5($password))) {
					if ($row['status'] == 'active') {
						// Set session and redirect to index.php on successful login
			...
			die("Query execution failed: " . mysqli_error($db_con));
		}
	}
}
?>
		...
		<div class="row animate__animated animate__pulse">
			<div class="col-md-4 offset-md-4">
				<form method="POST" action="">
					...
					<p>If you don't have a user account, you can <a href="register.php">Register Account!</a></p>
			</div>
			</form>
		</div>
	</div>
